...
if (rng_pool == null) {
    ...
    const onMouseMoveListener = function (ev: Event & { x: number; y: number; }) {
        ...
        if (count >= 256 || rng_pptr >= rng_psize) {
            if (modificationWindow.removeEventListener) {
                modificationWindow.removeEventListener("mousemove", onMouseMoveListener, false);
            } else if ((modificationWindow as any).detachEvent) {
                (modificationWindow as any).detachEvent("onmousemove", onMouseMoveListener);
            }
            return;
        }
        try {
    ...
    if (modificationWindow.addEventListener) {
        modificationWindow.addEventListener("mousemove", onMouseMoveListener, false);
    } else if ((modificationWindow as any).attachEvent) {
        (modificationWindow as any).attachEvent("onmousemove", onMouseMoveListener);
    }
